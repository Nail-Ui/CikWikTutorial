...
public class CatController : MonoBehaviour
{
    ...
    [SerializeField] private float _chaseDistanceTreshHold = 1.5f;
    ...
    private void Update()
    {
        if(_playerController.CatCanChase())
        {
            SetChaseMovement();
    ...
    private void SetChaseMovement()
    {
        ...
        Vector3 offsetPosition = _playerTransform.position - directionToPlayer * _chaseDistanceTreshHold;
    ...
    private void SetRandomDestination()
    {
        ...
        if(!destinationSet)
        {
            Debug.LogWarning("Failed to find valid destination");
            _isWaiting = true;
            _timer = _waitTime * 2;
        }
    }
    ...
    {
        Vector3 pos = (_initialPosition != Vector3.zero) ? _initialPosition : transform.position; // "?" bu ifade şu anlama geliyor eğer solda olan ifade doğruysa _InitialPosition u al değilse transform.position u al. Bu bir test 
